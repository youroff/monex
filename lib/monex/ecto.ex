defmodule MonEx.Ecto do
  ...
  and a Multi result repacker. Requires the consumer app to depend on `:ecto`.
  ...
  This injects `one_option/2`, `get_option/3`, `get_by_option/3` and exposes
  `repack_multi/1,2` on the Repo. The `*_option` helpers wrap the standard
  `nil`-returning Repo callbacks into `MonEx.Option`:

      MyApp.Repo.get_option(User, user_id)
      |> map(&do_something/1)
      |> get_or_else(default_user())

  `repack_multi/2` normalizes the four-tuple returned by `Repo.transaction/1`
  on a failed `Ecto.Multi` into a `MonEx.Result`.
  """
  ...
  @doc """
  Repack result of `Repo.transaction/1` on an `Ecto.Multi`.

  Supported options:
    * `:error`  — `:all` (default) | `:operation` | `:value` | `:changes`
    * `:result` — `:all` (default, full map) | `op_name` (returns just that op's value)
